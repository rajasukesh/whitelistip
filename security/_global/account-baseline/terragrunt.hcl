...
terraform {
  source = "${local.source_base_url}?ref=v0.35.1"
...
# ---------------------------------------------------------------------------------------------------------------------
locals {
  source_base_url = "git::git@github.com:gruntwork-io/terraform-aws-cis-service-catalog.git//modules/landingzone/account-baseline-security"
...
inputs = {
  ...
  macie_buckets_to_analyze = {
    (local.region_vars.locals.aws_region) = [local.region_vars.locals.state_bucket]
  }
