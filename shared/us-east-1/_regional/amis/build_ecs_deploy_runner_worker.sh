...
# https://github.com/gruntwork-io/terraform-aws-service-catalog/blob/v0.90.2/modules/mgmt/ecs-deploy-runner/ecs-deploy-runner-worker-al2.pkr.hcl
#
...
readonly PACKER_TEMPLATE_REPO_REF="v0.90.3"
readonly SERVICE_CATALOG_REF="v0.90.3"
readonly DEPLOY_RUNNER_REGION="us-east-1"
...
qa_account_id="$(jq -r '."qa".id' "$git_repo_root/accounts.json")"
ami_account_ids="[\"$dev_account_id\",\"$logs_account_id\",\"$prod_account_id\",\"$security_account_id\",\"$stage_account_id\", \"$qa_account_id\"]"
...
function run {
  ...
  args=( \
    ...
    --var instance_type="t2.micro"  # us-east-1 region does not have t3.micro available in some AZs, so we fall back to t2.micro
  )
