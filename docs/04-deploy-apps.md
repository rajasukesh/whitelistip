...
1. Open [`repos.yml` in
`shared/us-east-1/_regional/ecr-repos`](../shared/us-east-1/_regional/ecr-repos/repos.yml) and
...
```
git add shared/us-east-1/shared/data-stores/ecr-repos/terragrunt.hcl && git commit -m 'Added simple-web-app repo' && git push
```
...
with ID `980279442813`, an ECR repo in `us-east-1`, and an app called `simple-web-app`, the registry URL would be:

```
980279442813.dkr.ecr.us-east-1.amazonaws.com/simple-web-app
```

You can create a Docker image for this repo, with a `v1` label, as follows:

```bash
docker tag simple-web-app:latest 980279442813.dkr.ecr.us-east-1.amazonaws.com/simple-web-app:v1
```

Next, authenticate your Docker client with ECR in the shared-services account:

```bash
aws ecr get-login-password --region "us-east-1"  | docker login --username AWS --password-stdin 980279442813.dkr.ecr.us-east-1.amazonaws.com
```

And finally, push your newly tagged image to publish it:

```bash
docker push 980279442813.dkr.ecr.us-east-1.amazonaws.com/simple-web-app:v1
```
...
To set up a new ALB, you'll need to create a `terragrunt.hcl` in each app environment (that is, in dev, stage, and prod). For example, for the `stage` environment, create an `alb-simple-web-app` folder in [stage/us-east-1/networking/](../stage/us-east-1/stage/networking). Next, you can copy over the contents of the [alb terragrunt.hcl](../stage/us-east-1/stage/networking/alb/terragrunt.hcl) so you have something to start with.
...
* In the `container_definitions` object, set `image` to the repo url of the just published Docker image: e.g., `980279442813.dkr.ecr.us-east-1.amazonaws.com/simple-web-app`
...
1. Create a `simple-web-app` folder in [dev/us-east-1/dev/services](../dev/us-east-1/dev/services).
1. Copy over the contents of the [sample-app-frontend terragrunt.hcl](../dev/us-east-1/dev/services/sample-app-frontend/terragrunt.hcl).
...
```
aws --region us-east-1 ecs list-services --cluster <cluster-name>
```
...
```
aws --region us-east-1 ecs describe-services --cluster <cluster-name> --services <service-name>
```
