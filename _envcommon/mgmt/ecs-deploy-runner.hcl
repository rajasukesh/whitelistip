...
locals {
  ...
  git_ssh_private_key_secrets_manager_arn = "arn:aws:secretsmanager:us-east-1:980279442813:secret:MachineUserSSHPrivateKey-1QL6Je"
  github_pat_secrets_manager_arn          = "arn:aws:secretsmanager:us-east-1:980279442813:secret:MachineUserGitHubPAT-V7AWYW"
...
inputs = {
  ...
  # m5.2xlarge is not available in us-east-1e, so we restrict it to the first two subnets in the list
  private_subnet_ids = [
    dependency.vpc_mgmt.outputs.private_subnet_ids[0],
    dependency.vpc_mgmt.outputs.private_subnet_ids[1],
  ]

  shared_secrets_enabled     = true
  shared_secrets_kms_cmk_arn = "arn:aws:kms:us-east-1:980279442813:alias/shared-secrets"
  ...
  terraform_planner_config = {
    ...
    infrastructure_live_repositories_regex  = ["git@github\\.com:Furniture-com/infrastructure-live\\.git"]
  ...
  terraform_applier_config = {
    ...
    allowed_apply_git_refs                 = ["master", "origin/master", "main", "origin/main", ]
    machine_user_git_info = {
      name  = "fc-sa-github"
      email = "fc-sa-github@furniture.com"
    }
    repo_access_ssh_key_secrets_manager_arn = local.git_ssh_private_key_secrets_manager_arn
    repo_access_https_tokens = {
      github_token_secrets_manager_arn = local.github_pat_secrets_manager_arn
  ...
  ec2_worker_pool_configuration = {
    ami_filters = {
      ...
      filters = [
        {
          name   = "name"
          values = ["ecs-deploy-runner-worker-v0.90.2-*"]
        },
      ]
    }
