...
terraform {
  source = "${local.source_base_url}?ref=v0.35.0"
...
# ---------------------------------------------------------------------------------------------------------------------
locals {
  source_base_url = "git::git@github.com:gruntwork-io/terraform-aws-cis-service-catalog.git//modules/landingzone/account-baseline-app"
...
inputs = {
  ...
  kms_grants = (
    ...
    ? {
      ami_encryption_key = {
        kms_cmk_arn       = "arn:aws:kms:us-east-1:980279442813:alias/ami-encryption"
  ...
  macie_buckets_to_analyze = {
    (local.region_vars.locals.aws_region) = concat(
      [local.region_vars.locals.state_bucket],
      (
        local.account_name == "logs"
        ? [local.common_vars.locals.config_s3_bucket_name, local.common_vars.locals.cloudtrail_s3_bucket_name]
        : []
      ),
    ),
  }
